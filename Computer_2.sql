...
CREATE TABLE IF NOT EXISTS `Computer_2`.`Demon` (
  ...
  `rank` ENUM('SS', 'S', 'A', 'B', 'C') NOT NULL COMMENT '악마의 위험도\n초기값 C',
  `status` ENUM('ALIVE', 'DEAD') NOT NULL,
  `bounty` BIGINT NULL COMMENT '현상금\n초기값 null\n현상금 초깃값 팀원들과 상의',
...
CREATE TABLE IF NOT EXISTS `Computer_2`.`Hunter` (
  ...
  `name` VARCHAR(100) NOT NULL,
  `status` ENUM('ALIVE', 'DEAD') NOT NULL COMMENT '생존상태\n현상금 분배 시 이 값이 DEAD면 분배 하지 않음',
  `demon_id` BIGINT NULL COMMENT '계약한 악마의 PK\n계약 하지 않으면 null',
  `contract_cost` VARCHAR(100) NULL COMMENT '계약 체결 비용(대가)\n계약을 체결하지 않으면 null',
  `contract_power` VARCHAR(100) NULL COMMENT '계약에 따라 얻는 힘(능력)\n계약을 체결하지 않으면 null',
  `contract_date` DATE NULL COMMENT '계약 체결 날짜\n계약을 체결하지 않으면 null',
  PRIMARY KEY (`hunter_id`),
  INDEX `team_ID_idx` (`team_id` ASC) INVISIBLE,
  INDEX `demon_id_idx` (`demon_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Computer_2`.`Reward` (
  ...
  `mission_id` BIGINT NULL COMMENT 'Mission의 복합키',
  `battle_seq` BIGINT NULL COMMENT 'Mission의 복합키',
  `team_id` BIGINT NOT NULL,
  `total_amount` BIGINT NOT NULL COMMENT '총현상금',
  PRIMARY KEY (`reward_id`),
  UNIQUE INDEX `claim_id_UNIQUE` (`reward_id` ASC) VISIBLE,
  INDEX `mission_id_idx` (`mission_id` ASC) VISIBLE,
  INDEX `battle_seq_idx` (`battle_seq` ASC) VISIBLE,
  INDEX `team_id_idx` (`team_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Computer_2`.`Reward_Distribution` (
  `reward_id` BIGINT NOT NULL COMMENT '복합키\n{reward_id(Fk->Reward),\nhunter_id(FK->Hunter)}',
  `hunter_id` BIGINT NOT NULL COMMENT '복합키\n{reward_id(Fk->Reward),\nhunter_id(FK->Hunter)}',
